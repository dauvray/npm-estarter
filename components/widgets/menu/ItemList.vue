<template>
    ...
    <ul class="estarter-menu navbar-nav">
        <li class="nav-item" v-for="(item, idx) in this.items" :class="{ dropdown: item.children.length }" :key="idx">
            ...
            <!-- has children -->
            <a class="nav-link dropdown-toggle" :class="item.extras.class" data-toggle="dropdown" href="#" id="themes" aria-expanded="false"
               v-if="item.children.length"> {{ item.name }}</a>

            <div v-if="item.children.length" class="dropdown-menu" aria-labelledby="themes">
                <a v-for="(child, idx) in item.children" @click="goto" :data-path="child.link"
                   class="dropdown-item" :class="item.extras.class" > {{ child.name }}</a>
            </div>
